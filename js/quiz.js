...
(function () {
    ...
    root().innerHTML = `
      <div class="qz-game">
        ...
        <div class="qz-card" id="qzCard">
          <span class="qz-cat-tag">${meta.emoji || ""} ${esc(meta.name)}</span>
  ...
  function renderResult() {
    S.done = true;
    try {
      const n = (parseInt(localStorage.getItem("vh_quiz_plays") || "0", 10) || 0) + 1;
      localStorage.setItem("vh_quiz_plays", String(n));
    } catch (e) {}
    ...
    qs("#qzAgain").addEventListener("click", () => { sndTick(); startGame(S.cat); });
